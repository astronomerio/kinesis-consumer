 ...
 * @param {Function} _processRecord Function that will be passed each record and the checkpointer.
 */

function createConsumer(name, _processRecord) {
  assert(name, 'expected consumer name');
  assert(_processRecord, 'expected _processRecord function name');

  const processRecordQueue = async.queue(_processRecord, 1);
